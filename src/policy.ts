...
export async function policySync(
  ...
  const diff = simpleDiff(currentText, desired);

  if (!diff) return { changed: false, validated: true, written: false, etag: current.etag, diff: '' };
  const candidate = JSON.parse(desired) as PolicyDocument;
  await api.validatePolicy(candidate);
...
export function policyFromEnv(): string | undefined {
  return process.env.TS_POLICY_FILE || process.env.TS_POLICY;
}
